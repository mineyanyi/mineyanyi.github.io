var model = {
    //boadSize: the size of the grid for the board
    boardSize: 7,
    // numShips: the number of ships in the game
    numShips: 3,
    // shipLenghtz: the number of locations in each ship
    shipLenght: 3,
    // shipsSunk: how many ships have been sunk
    shipsSunk:0,

    //ships: the ships locations and hits
    ships:[
        {location: [0, 0, 0], hits: ["", "", ""] },
        {location: [0, 0, 0], hits: ["", "", ""] },
        {location: [0, 0, 0], hits: ["", "", ""] },
    ...
    // original hard-coded locations
    /*ships:[
        {location: ["06", "16", "24"], hits: ["", "", ""] },
    ...
    generateShip: function() {
        ...
        if (direction === 1) {
            ...
            (this.boardSize - this.shipLenght + 1));
        }
        else {
...
function parseGuess(guess) {
    ...
    if (guess === null || guess.length !== 2) {
        ...
        || column >= model.boardSize) {
            alert("Oops, this is off the board!");
        }
        else {
            return row + column;
        }
